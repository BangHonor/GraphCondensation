...
class GCN(nn.Module):
    ...
    def train_test_acc(self, args, features, adj, labels, flag='train'):
        if os.path.exists('/home/xzb/GCond/saved_ours/adj_'+flag+'_'+str(args.dataset)+'_'+str(args.seed)):
            adj=torch.load(f'/home/xzb/GCond/saved_ours/adj_{flag}_{args.dataset}_{args.seed}.pt')
        else:
            adj=utils.to_tensor1(adj=adj)
            if utils.is_sparse_tensor(adj):
            ...
            torch.save(adj, f'/home/xzb/GCond/saved_ours/adj_{flag}_{args.dataset}_{args.seed}.pt')
        #应该读取文件

        output = self.inference(features, adj).to(self.device)#features、adj:numpy
        loss_train = F.nll_loss(output, labels)
        acc_train = utils.accuracy(output, labels)
        print(flag+" set results:",#put the full graph into the fitted gcn
